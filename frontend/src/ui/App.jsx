...
const playAudio = (base64Audio) => {
  ...
  const handleTextSubmit = async (e) => {
    ...
    const userMessage = { id: Date.now(), user: 'Ryan', text: input, time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) };
    setMessages(prev => [...prev, userMessage]);
    setInput('');
    setIsLoading(true);

    try {
      const response = await axios.post('https://helloelia.com/text', { text: input });
  ...
  const handleImageUpload = async (e) => {
    ...
    const userMessage = { id: Date.now(), user: 'Ryan', text: `Image sent: ${file.name}`, time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) };
    setMessages(prev => [...prev, userMessage]);
    setIsLoading(true);
    ...
    try {
      const response = await axios.post('https://helloelia.com/text', formData);
      const { response: eliaText, audio } = response.data;
      const eliaMessage = { id: Date.now(), user: 'ELIA', text: eliaText, time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) };
    ...
    if (isRecording) {
      ...
      try {
        ...
        mediaRecorderRef.current.onstop = async () => {
          ...
          const userMessage = { id: placeholderId, user: 'Ryan', text: 'Voice Massage Sent.', time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) };
          setMessages(prev => [...prev, userMessage]);
